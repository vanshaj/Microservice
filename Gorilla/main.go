...
import (
	...
	"github.com/vanshaj/Microservice/Gorilla/handlers"
)
...
func main() {
	...
	sm := mux.NewRouter()
	getRouter := sm.Methods("GET").Subrouter() // registers a new route with a matcher for HTTP methods. and creates a subrouter for the route
	getRouter.HandleFunc("/", hh.GetProducts)

	putRouter := sm.Methods("PUT").Subrouter()
	putRouter.HandleFunc("/{id:[0-9]+}", hh.UpdateProducts)
	putRouter.Use(hh.MiddlewareProductHandler)

	postRouter := sm.Methods("POST").Subrouter()
	postRouter.HandleFunc("/", hh.AddProduct)
	postRouter.Use(hh.MiddlewareProductHandler)
